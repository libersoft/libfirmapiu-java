...
abstract class AbstractCadesBESCommandInterface<K, V, W> implements
	 ...
	 * 
	 */
	AbstractCadesBESCommandInterface(CRToken signToken, CRToken verifyToken,
	 ...
	 * 
	 * @see
	 * it.libersoft.firmapiu.CommandInterface#sign(it.libersoft.firmapiu.Data)
	 */
	@Override
	public ResultInterface<K, V> sign(Data<K> data) throws FirmapiuException {
		...
		if (this.signToken == null)
			throw new FirmapiuException(CRT_TOKEN_NOTFOUND,
					new NullPointerException("signToken=null"));
		...
		CMSSignedDataResultInterface<K, V> resultInterface = this
				.getCMSSIgnedDataResultInterface();

		// itera sul set dei dati da firmare e salva i dati firmati nel
		// resultInterface
		Set<K> dataSet = data.getDataSet();
		Iterator<K> itr = dataSet.iterator();
		// TODO vedere se signer può essere inizializzato dentro il while per
		// ottimizzare
		CadesBESSigner signer = new CadesBESSigner(this.signToken,
				this.digestCalculatorProviderStr);
		while (itr.hasNext()) {
			K dataKey = itr.next();
			try {
				CMSTypedData cmsData = new CMSProcessableByteArray(
						data.getArrayData(dataKey));
				...
				FirmapiuException fe1 = new FirmapiuException(
						SIGNER_CADESBES_ERROR, e);
				resultInterface.putFirmapiuException(dataKey, fe1);
			} catch (ProviderException e) {
				// questa eccezione potrebbe essere lanciata se si rimuove il
				// token pkcs11 durante il processo di firma
				// se il token è PKCS11token si slogga e rilancia l'eccezione
				// firmapiuexception al chiamante
	 ...
	 * 
	 * @see
	 * it.libersoft.firmapiu.CommandInterface#verify(it.libersoft.firmapiu.Data)
	 */
	@Override
	public ResultInterface<K, CMSReport> verify(Data<K> data)
		...
		if (this.verifyToken == null)
			throw new FirmapiuException(CRT_TOKEN_NOTFOUND,
					new NullPointerException("verifyToken=null"));

		// Carica il keystore contenenti i certificati di root delle CA ritenuti
		// affidabili per le operazioni di verifica
		this.verifyToken.loadKeyStore(null);

		// crea il resultset da restituire in risposta
		CMSReportResultInterface<K> resultInterface = this
				.getCMSReportResultInterface();

		// per ogni dato contenuto in data verifica che la firma sia corretta
		// (l'implmementazione concreta di data
		...
		while (dataItr.hasNext()) {
			...
			try {
				...
				// crea il verificatore per verificare la signedData ed effettua
				// tutte le verifiche su tutti i firmatari
				CadesBESVerifier verifier = new CadesBESVerifier(cmsSignedData,
						this.verifyToken);
				resultInterface.put(dataKey, verifier);
			} catch (CMSException e) {
				FirmapiuException fe1 = new FirmapiuException(
						CONTENT_CADESBES_ENCODINGERROR_ATTACHED, e);
				resultInterface.putFirmapiuException(dataKey, fe1);
			} catch (FirmapiuException e) {
				resultInterface.putFirmapiuException(dataKey, e);
			}
	 ...
	 * 
	 * @see
	 * it.libersoft.firmapiu.cades.CadesBESCommandInterface#verifyP7S(it.libersoft
	 * .firmapiu.cades.P7SData)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * it.libersoft.firmapiu.CommandInterface#getContentSignedData(it.libersoft
	 * .firmapiu.Data)
	 */
	@Override
	public ResultInterface<K, V> getContentSignedData(Data<K> signedData)
		...
		// crea il resultset da restituire in risposta
		CMSTypedDataResultInterface<K, V> resultInterface = this
				.getCMSTypedDataResultInterface();

		Set<K> dataSet = signedData.getDataSet();
		// per dato presente in signedData cerca di restituire il contenuto
		// originale della busta crittografica
		Iterator<K> dataItr = dataSet.iterator();
		while (dataItr.hasNext()) {
			// crea la busta crittografica dai dati di input
			K dataKey = dataItr.next();
			try {
				byte[] b = signedData.getArrayData(dataKey);
				CMSSignedData cmsSignedData = new CMSSignedData(b);
				// recupera il contenuto originale dei file da cmsSignedData
				resultInterface.put(dataKey, cmsSignedData.getSignedContent());
			} catch (CMSException e) {
				FirmapiuException fe1 = new FirmapiuException(
						CONTENT_CADESBES_ENCODINGERROR_ATTACHED, e);
				resultInterface.putFirmapiuException(dataKey, fe1);
